...
The system SHALL maintain a monthly CSV with accumulated counts for `MARCA_ITV`, `MODELO_ITV`, and `COD_PROVINCIA_VEH` combinations.
...
- **THEN** it includes columns `MARCA_ITV,MODELO_ITV,COD_PROVINCIA_VEH,CILINDRADA_ITV,COUNT`
- **AND** each row represents a unique `MARCA_ITV` + `MODELO_ITV` + `COD_PROVINCIA_VEH` combination
...
- **AND** in case of ties, by `COD_PROVINCIA_VEH`
